...
//
			...
			while (in.nextToken() != StreamTokenizer.TT_EOF) {
				...
				if (l.jinstr!=-1) {
					...
					//System.out.println(JopInstr.name(l.jinstr));
					if (JopInstr.name(l.jinstr).equals("jopsys_initctrl")) {
					jtbl.write("\t\twhen others => addr <= \""+bin(noim_address, ADDRBITS)+
